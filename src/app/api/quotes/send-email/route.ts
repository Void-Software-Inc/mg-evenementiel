...
export async function POST(request: Request) {
  try {
    const { first_name, last_name, email, phone_number, event_start_date, event_end_date, is_traiteur, description, pdfContent } = await request.json();
    ...
    // Define the static email address
    const staticEmail = 'mgevenementiel31@gmail.com'; // Static email address
...
const generatePDF = (pdfContent: any): Buffer => {
  ...
  doc.text(`Option traiteur: ${userInfo.is_traiteur ? 'Oui' : 'Non'}`, 10, 40);
  ...
  doc.text(`Date de l'événement: ${eventFromDate} au ${eventToDate}`, 10, 50);
  ...
  return Buffer.from(doc.output('arraybuffer')); // Return the PDF as a Buffer
};
